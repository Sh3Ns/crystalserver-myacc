...
class OTS_Player extends OTS_Row_DAO
{
    ...
    private $data = array(
        ...
        'hidden' => 0,
        'ismain' => 0,
        'world_id' => 1  // ADICIONADO: Define o mundo padrão como 1 (Crystal)
    );
    /**
     * Player skills.
     *
    ...
    public function save()
    {
        ...
        if (isset($this->data['id'])) {
            ...
            // NO MÉTODO save() DO OTS_Player.php
            // PROCURE PELA SEÇÃO "creates new player" (linha ~600-650)
            // E SUBSTITUA COMPLETAMENTE POR ESTE CÓDIGO:
            ...
            // ADICIONE world_id AQUI - SEMPRE
            if ($this->db->hasColumn('players', 'world_id')) {
                // Garante que world_id sempre tenha um valor válido
                $world_id_value = isset($this->data['world_id']) && $this->data['world_id'] > 0
                    ? $this->data['world_id']
                    : 1; // Default para world ID 1 se não definido
                $fields['world_id'] = $world_id_value;
            }
            ...
            foreach ($optionalColumns as $column => $relatedColumns) {
                if ($this->db->hasColumn('players', $column)) {
                    foreach ($relatedColumns as $relatedColumn) {
                        if (isset($this->data[$relatedColumn])) {
    ...
    {
        $this->data['world_id'] = (int) $world_id;
    }
